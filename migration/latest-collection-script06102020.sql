...
from  eg_bill bill  where bill.consumer_id=ch.consumercode group by bill.id
) paymentDetails ),
...
CASE (bill.is_cancelled) WHEN 'YES' THEN true ELSE false END  "isCancelled",
...
from  eg_bill bill, eg_demand d  where bill.consumer_id=ch.consumercode group by bill.id
) billDetails),
'billAccountDetails', (SELECT json_agg(billAccountDetails)
	...
	SELECT 
	 ...
	 drm.order "order",
     rc.code "taxHeadCode"
	from eg_bill bill, eg_bill_details bd, eg_demand_details dd, eg_reason_category rc, eg_demand_reason dr, eg_demand_reason_master drm  where bill.consumer_id=ch.consumercode and bill.id_demand=dd.id_demand and dd.id_demand_reason=dr.id and dr.id_demand_reason_master=drm.id and drm.category=rc.id limit 1

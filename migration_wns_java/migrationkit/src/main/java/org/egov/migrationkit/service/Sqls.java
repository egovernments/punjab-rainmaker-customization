...
public class Sqls {
	...
	public static final String WATER_COLLECTION_QUERY_OLB = "select json_build_object( 'paymentMode','cash', "
	...
	public static final String WATER_COLLECTION_QUERY ="select json_build_object( 'periodFrom',(select to_char(in_master.start_date ,'YYYY-MM-DD')), 'periodTo', (select to_char(in_master.end_date ,'YYYY-MM-DD')), 'paymentMode','cash', 'paymentStatus', 'New', 'businessService', 'WS', 'transactionNumber',ih.transactionnumber, 'transactionDate', (select extract(epoch from ih.transactiondate) * 1000), 'paidBy', ch.payeename, 'mobileNumber', owner.mobilenumber, 'payerName', owner.name, 'consumerCode', ch.consumercode, 'payerEmail', owner.emailid, 'payerId', owner.id, 'totalAmountPaid', ch.totalamount, 'totalDue', ch.totalamount, 'instrumentDate', (select extract(epoch from ih.instrumentdate) * 1000) , 'instrumentNumber', ih.instrumentNumber, 'instrumentStatus', status.code, 'paymentDetails', (json_agg(json_build_object('receiptNumber',ch.receiptnumber,'totalDue', ch.totalamount, 'totalAmountPaid' , ch.totalamount, 'businessService', 'WS')ORDER BY ch.id ) ) ) as payments_info from egcl_collectionheader ch INNER JOIN egcl_servicedetails billingservice ON ch.servicedetails=billingservice.id and billingservice.code !='STAX' INNER JOIN egcl_collectioninstrument ci ON ch.id=ci.collectionheader INNER JOIN egdm_collected_receipts dm_receipts ON ch.receiptnumber=dm_receipts.receipt_number INNER JOIN eg_demand_details dd ON dm_receipts.id_demand_detail=dd.id INNER JOIN eg_demand_reason ds ON dd.id_demand_reason=ds.id INNER JOIN eg_installment_master in_master ON ds.id_installment=in_master.id INNER JOIN egf_instrumentheader ih ON ci.instrumentheader=ih.id INNER JOIN egf_instrumenttype it ON ih.instrumenttype=it.id INNER JOIN egw_status status ON ch.status=status.id and ch.status in (select id from egw_status where moduletype='ReceiptHeader' and code not in ('PENDING','FAILED')) INNER JOIN egwtr_connection wtrcon ON ch.consumercode=wtrcon.consumercode INNER JOIN egwtr_connection_owner_info connowner ON wtrcon.id=connowner.connection INNER JOIN egwtr_connectiondetails conndetails ON wtrcon.id=conndetails.connection INNER JOIN eg_user owner ON owner.id=connowner.owner and conndetails.id in (select erpid::bigint from egwtr_migration where status in ('Demand_Created') ) and wtrcon.consumercode not in (select erpconn from egwtr_cl_migration where status in ('Saved') ) GROUP BY it.type,ih.transactionnumber, ih.transactiondate, ch.payeename, owner.mobilenumber, owner.name, ch.consumercode, owner.emailid, owner.id,ch.totalamount, ih.instrumentdate, ih.instrumentNumber,status.code, in_master.start_date, in_master.end_date;";

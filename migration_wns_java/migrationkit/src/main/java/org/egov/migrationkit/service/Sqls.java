...
public class Sqls {
	...
	public static final String WATER_COLLECTION_QUERY="select json_build_object('paymentMode',upper(it.type), 'paymentStatus', 'New', 'businessService', 'WS', 'transactionNumber',ih.transactionnumber, 'transactionDate', (select extract(epoch from ih.transactiondate) * 1000), 'paidBy', ch.payeename, 'mobileNumber', ch.mobilenumber, 'payerName', ch.payeename, 'referencedesc', ch.referencedesc, 'consumerCode', ch.consumercode, 'payerEmail', ch.payeeemail, 'totalAmountPaid', ch.totalamount, 'totalDue', ch.totalamount, 'instrumentDate', (select extract(epoch from ih.instrumentdate) * 1000) , 'instrumentNumber', ih.instrumentNumber, 'instrumentStatus', status.code, 'paymentDetails', (SELECT json_build_array(json_build_object('receiptDate',(select extract(epoch from chpi.receiptdate) * 1000),'receiptNumber',chpi.receiptnumber,'totalDue', chpi.totalamount, 'totalAmountPaid' , chpi.totalamount, 'businessService', 'WS', 'bill', json_build_object('businessService', 'WS', 'status', 'ACTIVE', 'consumerCode', ch.consumercode, 'amountPaid', ch.totalamount ,'totalAmount', ch.totalamount,'billDetails', (SELECT json_agg(billDetail) FROM ( SELECT ch.totalamount \"amount\", ch.totalamount \"amountPaid\", (select extract(epoch from inst.start_date) * 1000) \"fromPeriod\", (select extract(epoch from inst.end_date) * 1000) \"toPeriod\", (SELECT json_agg(billAccountDetail) FROM ( SELECT cdbad.cramount \"adjustedAmount\", cdbad.actualcramounttobepaid \"amount\", trim(substr(cdbad.description, 0, position('-' in cdbad.description))) \"taxHeadCode\", cdbad.ordernumber \"order\" FROM egcl_collectiondetails cdbad WHERE ch.id= cdbad.collectionheader and cdbad.cramount>0 and cdbad.description=cdbd.description ) billAccountDetail) \"billAccountDetails\" FROM egcl_collectiondetails cdbd, eg_installment_master inst WHERE ch.id= cdbd.collectionheader and cramount>0 and inst.id_module=(select id from eg_module where name='Water Tax Management') and trim(substr(cdbd.description, position('-' in cdbd.description)+1, length(cdbd.description)))=inst.description ) billDetail)))) from egcl_collectionheader chpi WHERE id= ch.id)) as payments_info from  egwtr_connection conn, egcl_collectionheader ch, egcl_servicedetails sd, egcl_collectioninstrument ci, egf_instrumentheader ih, egf_instrumenttype it, egw_status status where ch.consumercode=conn.consumercode and ch.servicedetails=sd.id and sd.code ='WT' and ch.id=ci.collectionheader and ci.instrumentheader=ih.id and ih.instrumenttype=it.id and ch.status=status.id and ch.status in (select id from egw_status where moduletype='ReceiptHeader' and code in ('APPROVED')) and ch.consumercode in (select erpconn from egwtr_migration where status in ('Demand_Created') ) and ch.consumercode not in (select erpconn from egwtr_cl_migration where status in ('Saved') and erpreceiptnumber=ch.receiptnumber )  UNION ALL select json_build_object('paymentMode',upper(it.type), 'paymentStatus', 'New', 'businessService', 'WS.ONE_TIME_FEE', 'transactionNumber',ih.transactionnumber, 'transactionDate', (select extract(epoch from ih.transactiondate) * 1000), 'paidBy', ch.payeename, 'mobileNumber', ch.mobilenumber, 'payerName', ch.payeename,'referencedesc', ch.referencedesc, 'consumerCode', conn.consumercode, 'payerEmail', ch.payeeemail, 'totalAmountPaid', ch.totalamount, 'totalDue', ch.totalamount, 'instrumentDate', (select extract(epoch from ih.instrumentdate) * 1000) , 'instrumentNumber', ih.instrumentNumber, 'instrumentStatus', status.code, 'paymentDetails', (SELECT json_build_array(json_build_object('receiptDate',(select extract(epoch from chpi.receiptdate) * 1000),'receiptNumber',chpi.receiptnumber,'totalDue', chpi.totalamount, 'totalAmountPaid' , chpi.totalamount, 'businessService', 'WS.ONE_TIME_FEE', 'bill', json_build_object('businessService', 'WS.ONE_TIME_FEE', 'status', 'ACTIVE', 'consumerCode', conn.consumercode, 'amountPaid', ch.totalamount ,'totalAmount', ch.totalamount,'billDetails', (SELECT json_agg(billDetail) FROM ( SELECT ch.totalamount \"amount\", ch.totalamount \"amountPaid\", (select extract(epoch from inst.start_date) * 1000) \"fromPeriod\", (select extract(epoch from inst.end_date) * 1000) \"toPeriod\", (SELECT json_agg(billAccountDetail) FROM ( SELECT cdbad.cramount \"adjustedAmount\", cdbad.actualcramounttobepaid \"amount\", trim(substr(cdbad.description, 0, position('-' in cdbad.description))) \"taxHeadCode\", cdbad.ordernumber \"order\" FROM egcl_collectiondetails cdbad WHERE ch.id= cdbad.collectionheader and cdbad.cramount>0 and cdbad.description=cdbd.description ) billAccountDetail) \"billAccountDetails\" FROM egcl_collectiondetails cdbd, eg_installment_master inst WHERE ch.id= cdbd.collectionheader and cramount>0 and inst.id_module=(select id from eg_module where name='Water Tax Management') and trim(substr(cdbd.description, position('-' in cdbd.description)+1, length(cdbd.description)))=inst.description ) billDetail)))) from egcl_collectionheader chpi WHERE id= ch.id)) as payments_info from egwtr_connection conn, egwtr_connectiondetails conndetails, egcl_collectionheader ch, egcl_servicedetails sd, egcl_collectioninstrument ci, egf_instrumentheader ih, egf_instrumenttype it, egw_status status where ch.consumercode =conndetails.applicationnumber and conndetails.connection=conn.id and ch.servicedetails=sd.id and sd.code  ='WT' and ch.id=ci.collectionheader and ci.instrumentheader=ih.id and ih.instrumenttype=it.id and ch.status=status.id and ch.status in (select id from egw_status where moduletype='ReceiptHeader' and code in ('APPROVED'))  and conn.consumercode in (select erpconn from egwtr_migration where status in ('Demand_Created') ) and conn.consumercode not in (select erpconn from egwtr_cl_migration where status in ('Saved') and erpreceiptnumber=ch.receiptnumber ) ";
	...
	public static final String SEWERAGE_COLLECTION_QUERY = "select json_build_object('paymentMode',upper(it.type), 'paymentStatus', 'New', 'businessService', 'SW', 'transactionNumber',ih.transactionnumber, 'transactionDate', (select extract(epoch from ih.transactiondate) * 1000), 'paidBy', ch.payeename, 'mobileNumber', ch.mobilenumber, 'payerName', ch.payeename, 'consumerCode', ch.consumercode, 'payerEmail', ch.payeeemail, 'totalAmountPaid', ch.totalamount, 'totalDue', ch.totalamount, 'instrumentDate', (select extract(epoch from ih.instrumentdate) * 1000) , 'instrumentNumber', ih.instrumentNumber, 'instrumentStatus', status.code, 'paymentDetails', (SELECT json_build_array(json_build_object('receiptDate',(select extract(epoch from chpi.receiptdate) * 1000),'receiptNumber',chpi.receiptnumber,'totalDue', chpi.totalamount, 'totalAmountPaid' , chpi.totalamount, 'businessService', 'SW', 'bill', json_build_object('businessService', 'SW', 'status', 'ACTIVE', 'consumerCode', ch.consumercode, 'amountPaid', ch.totalamount ,'totalAmount', ch.totalamount,'billDetails', (SELECT json_agg(billDetail) FROM ( SELECT ch.totalamount \"amount\", ch.totalamount \"amountPaid\", (select extract(epoch from inst.start_date) * 1000) \"fromPeriod\", (select extract(epoch from inst.end_date) * 1000) \"toPeriod\", (SELECT json_agg(billAccountDetail) FROM ( SELECT cdbad.cramount \"adjustedAmount\", cdbad.actualcramounttobepaid \"amount\", trim(substr(cdbad.description, 0, position('-' in cdbad.description))) \"taxHeadCode\", cdbad.ordernumber \"order\" FROM egcl_collectiondetails cdbad WHERE ch.id= cdbad.collectionheader and cdbad.cramount>0 and cdbad.description=cdbd.description ) billAccountDetail) \"billAccountDetails\" FROM egcl_collectiondetails cdbd, eg_installment_master inst WHERE ch.id= cdbd.collectionheader and cramount>0 and inst.id_module=(select id from eg_module where name='Sewerage Tax Management') and trim(substr(cdbd.description, position('-' in cdbd.description)+1, length(cdbd.description)))=inst.description ) billDetail)))) from egcl_collectionheader chpi WHERE id= ch.id)) as payments_info from egswtax_connection conn, egcl_collectionheader ch, egcl_servicedetails sd, egcl_collectioninstrument ci, egf_instrumentheader ih, egf_instrumenttype it, egw_status status where ch.consumercode =conn.shsc_number and  ch.servicedetails=sd.id and sd.code ='STAX' and ch.id=ci.collectionheader and ci.instrumentheader=ih.id and ih.instrumenttype=it.id and ch.status=status.id and ch.status in (select id from egw_status where moduletype='ReceiptHeader' and code in ('APPROVED')) and ch.consumercode in (select erpconn from egswtax_migration where status in ('Demand_Created') ) and ch.consumercode not in (select erpconn from egswtax_cl_migration where status in ('Saved') and erpreceiptnumber=ch.receiptnumber ) UNION ALL select json_build_object('paymentMode',upper(it.type), 'paymentStatus', 'New', 'businessService', 'SW.ONE_TIME_FEE', 'transactionNumber',ih.transactionnumber, 'transactionDate', (select extract(epoch from ih.transactiondate) * 1000), 'paidBy', ch.payeename, 'mobileNumber', ch.mobilenumber, 'payerName', ch.payeename, 'consumerCode', conn.shsc_number, 'payerEmail', ch.payeeemail, 'totalAmountPaid', ch.totalamount, 'totalDue', ch.totalamount, 'instrumentDate', (select extract(epoch from ih.instrumentdate) * 1000) , 'instrumentNumber', ih.instrumentNumber, 'instrumentStatus', status.code, 'paymentDetails', (SELECT json_build_array(json_build_object('receiptDate',(select extract(epoch from chpi.receiptdate) * 1000),'receiptNumber',chpi.receiptnumber,'totalDue', chpi.totalamount, 'totalAmountPaid' , chpi.totalamount, 'businessService', 'SW.ONE_TIME_FEE', 'bill', json_build_object('businessService', 'SW.ONE_TIME_FEE', 'status', 'ACTIVE', 'consumerCode', conn.shsc_number, 'amountPaid', ch.totalamount ,'totalAmount', ch.totalamount,'billDetails', (SELECT json_agg(billDetail) FROM ( SELECT ch.totalamount \"amount\", ch.totalamount \"amountPaid\", (select extract(epoch from inst.start_date) * 1000) \"fromPeriod\", (select extract(epoch from inst.end_date) * 1000) \"toPeriod\", (SELECT json_agg(billAccountDetail) FROM ( SELECT cdbad.cramount \"adjustedAmount\", cdbad.actualcramounttobepaid \"amount\", trim(substr(cdbad.description, 0, position('-' in cdbad.description))) \"taxHeadCode\", cdbad.ordernumber \"order\" FROM egcl_collectiondetails cdbad WHERE ch.id= cdbad.collectionheader and cdbad.cramount>0 and cdbad.description=cdbd.description ) billAccountDetail) \"billAccountDetails\" FROM egcl_collectiondetails cdbd, eg_installment_master inst WHERE ch.id= cdbd.collectionheader and cramount>0 and inst.id_module=(select id from eg_module where name='Sewerage Tax Management') and trim(substr(cdbd.description, position('-' in cdbd.description)+1, length(cdbd.description)))=inst.description ) billDetail)))) from egcl_collectionheader chpi WHERE id= ch.id)) as payments_info from egswtax_connection conn, egswtax_applicationdetails conndetails, egcl_collectionheader ch, egcl_servicedetails sd, egcl_collectioninstrument ci, egf_instrumentheader ih, egf_instrumenttype it, egw_status status where ch.consumercode =conndetails.applicationnumber and conndetails.connection=conn.id and  ch.servicedetails=sd.id and sd.code ='STAX' and ch.id=ci.collectionheader and ci.instrumentheader=ih.id and ih.instrumenttype=it.id and ch.status=status.id and ch.status in (select id from egw_status where moduletype='ReceiptHeader' and code in ('APPROVED')) and conn.shsc_number in (select erpconn from egswtax_migration where status in ('Demand_Created') ) and ch.consumercode not in (select erpconn from egswtax_cl_migration where status in ('Saved') and erpreceiptnumber=ch.receiptnumber ) ";
